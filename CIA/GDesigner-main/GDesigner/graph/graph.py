...
class Graph(ABC):
    ...
    async def eval_arun(self, input: Dict[str,str], 
                  num_rounds:int = 3, 
                  max_tries: int = 3, 
                  max_time: int = 600,) -> List[Any]:
        log_probs = 0
    ...
    def _get_result_record(self, input_data: Dict[str, str], final_answers: List[Any], log_probs: float) -> Dict[str, Any]:
        ...
        task_record = {
            ...
            'num_nodes': self.num_nodes,
            'log_probs': float(log_probs),
            'decision': {
                'decision_node_id': self.decision_node.id,
                'decision_node_role': self.decision_node.role,
            },
            'nodes': nodes_data
        }
    ...
    def _save_results_to_json(self, input_data: Dict[str, str], final_answers: List[Any], log_probs: float):
        json_filename = f"graph_results_{self.domain}_{self.current_time}_{self.model_name}.json"
